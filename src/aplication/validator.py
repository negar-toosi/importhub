...
class ShipmentRecordValidator:
    ...
    def price(value) -> Optional[str]:
        ...
        if pd.isna(value):
            return "price is null"
        if not isinstance(value, (int, float, Decimal)):
            return "price is not a decimal number"
        try:
            d = Decimal(str(value))
        except InvalidOperation:
            return "price is not a valid decimal number"
        if d < 0:
    ...
    def delivery_date(value) -> Optional[str]:
        if value is None or pd.isna(value):
            return "delivery_date is null"
        ...
        if isinstance(value, str):
            try:
                ...
                return "delivery_date is not a valid date"
            return None
        return "delivery_date is not a valid date"
